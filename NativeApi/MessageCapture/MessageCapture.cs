...
public class MessageCapture : IDisposable
{
    ...
    public SafeWndHandle CaptureWndHandle { get; private set; } = null!;
    ...
    private void QuitCaptureWindow()
    {
        if (CaptureWndHandle.IsInvalid) return;
        if (SendMessage(CaptureWndHandle, (uint)WindowsMessage.Quit, IntPtr.Zero, IntPtr.Zero) != 0)
            CheckLastErrorWithException(nameof(QuitCaptureWindow));
    }
